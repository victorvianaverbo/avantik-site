...
(() => {
  ...
  /* ---------- 3. Countdown ----------
     A data vem do HTML (<div id="countdown" data-target="2026-08-13T14:00:00-03:00">),
     nao do JS: e a unica coisa que diferia entre os 5 script.js do Apogeu, e
     era o que obrigava a manter 5 copias do arquivo. */
  const cdWrap = document.getElementById('countdown');
  if (cdWrap) {
    const target = new Date(cdWrap.dataset.target || '2026-08-13T14:00:00-03:00').getTime();
    const els = {
  ...
  if (canvas && !reduceMotion) {
    ...
    // Cor das particulas lida do CSS (--av-gold-rgb no :root do evento.css),
    // pra nao existir um segundo lugar de verdade para o dourado.
    const goldRGB = getComputedStyle(document.documentElement)
      .getPropertyValue('--av-gold-rgb').trim() || '227, 179, 65';
