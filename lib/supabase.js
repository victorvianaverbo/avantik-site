...
export const TIERS = {
  ...
  profissional: {
    ...
    features: [
      'Tudo do Essencial',
      '10 fotos no perfil',
      'Todos os temas de atuação',
      '1 vídeo',
      'WhatsApp visível para empresas',
      'Selo verificado',
      'Prioridade na busca',
      'Programa Avantik Pro (basico)',
    ]
  },
  destaque: {
    name: 'Destaque',
...
export async function getUserType(userId) {
  ...
  const [contractorRes, speakerRes] = await Promise.all([
    supabase.from('contractors').select('*').eq('auth_id', userId).single(),
    supabase.from('speakers').select('*').eq('auth_id', userId).single(),
  ]);
